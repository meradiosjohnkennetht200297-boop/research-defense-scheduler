...
function monthMeta(monthKey: string) {
  ...
  return { year, month, days, startWeekday, firstDate: `${monthKey}-01`, lastDate }
}
        ...
        <div className={`container ${styles.calendarWrap}`}>
          <div className={styles.calendarHeading}>
            <div>
              ...
              <p>Select a marked date to view the published defenses scheduled that day.</p>
            </div>
          </div>
          ...
          <div className={`card ${styles.calendarCard}`}>
            ...
            <div className={styles.calendarGrid}>
              {cells.map((cell, index) => {
                ...
                if (hasSchedule) {
                  return (
                    <Link
                      className={`${styles.day} ${styles.activeDay}${isToday ? ` ${styles.today}` : ''}`}
                      href={`/schedule?date=${cell.dateKey}`}
                      key={cell.dateKey}
                      title={`${count} published ${count === 1 ? 'defense' : 'defenses'}`}
                      aria-label={`${formatDate(cell.dateKey)}, ${count} published ${count === 1 ? 'defense' : 'defenses'}`}
                    >
                      <span>{cell.day}</span>
                      <i aria-hidden="true" />
                    </Link>
                ...
                return (
                  <span className={`${styles.day} ${styles.inactiveDay}${isToday ? ` ${styles.today}` : ''}`} key={cell.dateKey}>
                    <span>{cell.day}</span>
                  </span>
                )
              })}
            ...
            <div className={styles.calendarNote}>
              ...
              <span>Marked dates have published defenses.</span>
            </div>
          </div>
        </div>
      </section>
      ...
      <section className="section minimal-today-section">
        <div className="container">
          <div className="minimal-section-heading">
            ...
            {!todayResult.error ? <span>{schedules.length} {schedules.length === 1 ? 'defense' : 'defenses'}</span> : null}
          </div>

          {todayResult.error ? (
            <div className="minimal-empty"><h3>Today&apos;s defenses are temporarily unavailable.</h3><p>Please try again later.</p></div>
          ) : schedules.length === 0 ? (
            <div className="minimal-empty"><h3>No defenses scheduled today.</h3><p>Use the calendar above to select another marked date, or check your Research ID for your group&apos;s status.</p></div>
          ) : (
              ...
              {schedules.map((schedule) => {
                ...
                return (
                  <article className="minimal-defense-card" key={schedule.id}>
                    <div className="minimal-defense-time"><strong>{formatTime(schedule.start_time)}</strong><span>– {formatTime(schedule.end_time)}</span></div>
                    <div className="minimal-defense-content">
                      ...
                      <h3>{stage.title_snapshot}</h3>
                      <p className="minimal-venue">{schedule.venue}</p>
                      <small>{group.public_code}</small>
                    </div>
