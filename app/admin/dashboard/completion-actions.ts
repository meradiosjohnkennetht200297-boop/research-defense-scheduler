...
export async function confirmDefenseCompleted(formData: FormData) {
  ...
  if (error) {
    ...
    redirect('/admin/dashboard?error=Unable%20to%20confirm%20the%20defense%20as%20completed.')
  }

  const result = (data ?? {}) as { ok?: boolean; error?: string }
  if (!result.ok) {
    redirect(`/admin/dashboard?error=${encodeURIComponent(result.error || 'Unable to confirm the defense as completed.')}`)
  }
  ...
  redirect('/admin/dashboard?confirmed=1#action-required')
}
