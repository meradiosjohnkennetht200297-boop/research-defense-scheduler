...
type GroupData = { id: string; title: string; program: string | null; major: string | null; research_design: string | null; research_design_other: string | null; defense_type: string | null; contact_person: string; contact_email: string | null; contact_number: string | null; research_file_url: string | null; instructor_id: string | null; adviser_id: string | null }
...
export default function EditResearchForm({ group, faculty, memberNames }: { group: GroupData; faculty: FacultyRow[]; memberNames: string[] }) {
  ...
  return <form action={saveResearchGroupEdits} className="card schedule-form workspace-refined-form">
    <input name="groupId" type="hidden" value={group.id} />

    <section className="workspace-form-section">
      <div className="workspace-section-heading"><div><h3>Research information</h3><p>Update the current research details. The defense stage is fixed for this request.</p></div></div>
      <div className="field-grid workspace-schedule-fields">
        ...
        <div className="field"><label htmlFor="major">Major</label><select defaultValue={group.major ?? ''} id="major" name="major"><option value="">Not applicable</option><optgroup label="BSED"><option value="English">English</option><option value="Filipino">Filipino</option><option value="Mathematics">Mathematics</option><option value="Science">Science</option></optgroup><optgroup label="BSBA"><option value="MM">MM</option><option value="FM">FM</option><option value="HRM">HRM</option></optgroup></select></div>
        <div className="field"><label htmlFor="researchDesign">Research design</label><select defaultValue={group.research_design ?? ''} id="researchDesign" name="researchDesign"><option value="">Not specified</option>{RESEARCH_DESIGN_OPTIONS.map((option) => <option key={option.value} value={option.value}>{option.label}</option>)}</select></div>
        <div className="field"><label htmlFor="researchDesignOther">Other research design</label><input defaultValue={group.research_design_other ?? ''} id="researchDesignOther" maxLength={120} name="researchDesignOther" /><p className="field-help">Complete this only when Research design is Other.</p></div>
        <div className="field"><label>Defense stage</label><input readOnly value={defenseLabel(group.defense_type)} /></div>
      </div>
    </section>

    <section className="workspace-form-section">
      <div className="workspace-section-heading"><div><h3>Group and faculty</h3><p>Edit members and assigned research faculty for the current stage.</p></div></div>
      <div className="field"><label htmlFor="members">Group members <span className="required-mark">*</span></label><textarea defaultValue={memberNames.join('\n')} id="members" name="members" required rows={Math.min(10, Math.max(5, memberNames.length + 1))} /><p className="field-help">One member per line. Maximum of 20 members.</p></div>
    ...
    <section className="workspace-form-section">
      <div className="workspace-section-heading"><div><h3>Contact and research file</h3><p>Update administrative contact details and the private research-file link.</p></div></div>
    ...
    <div className="workspace-form-actions"><div className="workspace-save-note"><strong>Current defense stage only</strong><span>Saving here synchronizes the current stage snapshot but does not change schedule, panel, or completion history.</span></div><div className="workspace-action-buttons"><Link className="button button-secondary" href={`/admin/groups/${group.id}`}>Cancel</Link><button className="button workspace-save-button" type="submit">Save Changes</button></div></div>
  </form>
}
