...
export async function saveDefenseSchedule(formData:FormData){
 ...
 if(!defense.defense_type){if(!['title','proposal','final'].includes(legacyDefenseType))groupError(groupId,'Select the correct defense type for this legacy record.');const[stageUpdate,groupUpdate]=await Promise.all([supabase.from('research_defenses').update({defense_type:legacyDefenseType}).eq('id',defenseId).is('defense_type',null),supabase.from('research_groups').update({defense_type:legacyDefenseType}).eq('id',groupId)]);if(stageUpdate.error||groupUpdate.error)groupError(groupId,'Unable to record the defense type for this legacy record.')}
