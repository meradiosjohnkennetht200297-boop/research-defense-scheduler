...
type ScheduleConflict={kind:'venue'|'faculty';public_code:string;title:string;venue:string|null;faculty_name?:string;existing_roles?:string[]}
...
export async function saveDefenseSchedule(formData:FormData){
 ...
 if(!groupId||!defenseId||!defenseDate||!startTime||!endTime||!chairId)groupError(groupId,'Date, start time, end time, and panel chair are required.');if(endTime<=startTime)groupError(groupId,'End time must be later than start time.')
