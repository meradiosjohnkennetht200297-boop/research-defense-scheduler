...
function ResearchRequestForm({ faculty, mode, defenseType, initial, access, onBack }: { faculty: SubmissionFaculty[]; mode: 'new' | 'continue'; defenseType: DefenseType; initial?: ExistingGroup; access?: Access; onBack: () => void }) {
  ...
  useEffect(() => { if (step > 1) panelRef.current?.focus() }, [step])
  ...
  function validateStep(target: Step) {
    ...
    if (target === 3) {
      ...
      if (values.researchFileUrl.trim() && !validDrive(values.researchFileUrl)) return 'Use a valid Google Drive or Google Docs research file link, or leave it blank.'
    }
    return ''
  }
  ...
  async function submit(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      setError(caught instanceof TypeError ? 'We could not confirm your submission. Check your connection. If you may have submitted already, contact the Research Office before trying again.' : caught instanceof Error ? caught.message : 'Unable to submit the defense request. Please try again.')
    } finally {
      setSubmitting(false)
    }
  }
  ...
  async function copyResearchCode() {
    if (!success) return
    try { await navigator.clipboard.writeText(success.researchCode); setCopied(true); setCopyError(false) } catch { setCopied(false); setCopyError(true) }
  }
  ...
  if (success) {
    return (
      <div className="card lifecycle-success" aria-live="polite">
        ...
        {copyError ? <p role="status">Copy was unavailable. Select the Research Code above and copy it, or write it down.</p> : null}<div className="submission-success-actions">
  ...
  return (
    <form className="card lifecycle-form lifecycle-wizard" aria-busy={submitting} onSubmit={submit}>
      ...
      <div className="wizard-progress" aria-label="Submission progress">
        {STEP_LABELS.map((label, index) => {
          ...
          return <div aria-current={step === number ? 'step' : undefined} className={`wizard-progress-step${step === number ? ' active' : ''}${step > number ? ' complete' : ''}`} key={label}><span>{step > number ? '✓' : number}</span><strong>{label}</strong></div>
        })}
      </div>
      ...
      {step === 1 ? (
        <section className="lifecycle-form-section wizard-panel" ref={panelRef} tabIndex={-1}><h3>Research</h3><div className="field-grid"><div className="field full"><label htmlFor={`${mode}-title`}>Research title <span className="required-mark">*</span></label><textarea id={`${mode}-title`} aria-required="true" aria-invalid={Boolean(error && !values.title.trim())} maxLength={500} onChange={(event) => setValue('title', event.target.value)} value={values.title} /></div><div className="field"><label htmlFor={`${mode}-program`}>Program <span className="required-mark">*</span></label><select id={`${mode}-program`} aria-required="true" aria-invalid={Boolean(error && !values.program)} onChange={(event) => setValue('program', event.target.value)} value={values.program}><option value="">Select program</option><option value="BEED">BEED</option><option value="BSED">BSED</option><option value="BSA">BSA</option><option value="BSAIS">BSAIS</option><option value="BSBA">BSBA</option></select></div>{majors.length ? <div className="field"><label htmlFor={`${mode}-major`}>Major <span className="required-mark">*</span></label><select id={`${mode}-major`} aria-required="true" aria-invalid={Boolean(error && !values.major)} onChange={(event) => setValue('major', event.target.value)} value={values.major}><option value="">Select major</option>{majors.map((major) => <option key={major}>{major}</option>)}</select></div> : null}<div className="field full"><label htmlFor={`${mode}-research-design`}>Research design <span className="required-mark">*</span></label><select id={`${mode}-research-design`} aria-required="true" aria-invalid={Boolean(error && !values.researchDesign)} onChange={(event) => setValue('researchDesign', event.target.value)} value={values.researchDesign}><option value="">Select research design</option>{RESEARCH_DESIGN_OPTIONS.map((option) => <option key={option.value} value={option.value}>{option.label}</option>)}</select></div>{values.researchDesign === 'other' ? <div className="field full"><label htmlFor={`${mode}-research-design-other`}>Specify research design <span className="required-mark">*</span></label><input id={`${mode}-research-design-other`} aria-required="true" maxLength={120} onChange={(event) => setValue('researchDesignOther', event.target.value)} value={values.researchDesignOther} /></div> : null}</div></section>
      ) : null}

      {step === 2 ? (
        <section className="lifecycle-form-section wizard-panel" ref={panelRef} tabIndex={-1}><div className="lifecycle-section-head"><h3>Group members</h3></div><div className="submission-member-list">{members.map((member, index) => <div className="submission-member-row" key={`${index}-${members.length}`}><div className="field"><label htmlFor={`${mode}-member-${index}`}>Member {index + 1}{index === 0 ? <span className="required-mark"> *</span> : null}</label><input id={`${mode}-member-${index}`} autoComplete="name" aria-required={index === 0} maxLength={150} onChange={(event) => setMembers((current) => current.map((value, i) => i === index ? event.target.value : value))} value={member} /></div><button aria-label={`Remove member ${index + 1}`} className="button button-secondary submission-remove-member" disabled={members.length === 1} onClick={() => setMembers((current) => current.filter((_, i) => i !== index))} type="button">−</button></div>)}</div><button className="button button-secondary button-small submission-add-member" disabled={members.length >= 20} onClick={() => setMembers((current) => [...current, ''])} type="button">+ Add Member</button><div className="field-grid lifecycle-faculty-grid"><RoleFacultySearch faculty={instructorFaculty} id={`${mode}-instructor`} label="Research instructor" onChange={(value) => setValue('instructorId', value)} value={values.instructorId} /><RoleFacultySearch faculty={adviserFaculty} id={`${mode}-adviser`} label="Research adviser" onChange={(value) => setValue('adviserId', value)} value={values.adviserId} /></div></section>
      ) : null}

      {step === 3 ? (
        <section className="lifecycle-form-section wizard-panel" ref={panelRef} tabIndex={-1}><h3>Contact & research file</h3><div className="field-grid"><div className="field"><label htmlFor={`${mode}-contact`}>Contact person <span className="required-mark">*</span></label><input id={`${mode}-contact`} autoComplete="name" aria-required="true" aria-invalid={Boolean(error && !values.contactPerson.trim())} maxLength={150} onChange={(event) => setValue('contactPerson', event.target.value)} value={values.contactPerson} /></div><div className="field"><label htmlFor={`${mode}-email`}>Email <span className="optional-mark">Optional</span></label><input id={`${mode}-email`} autoComplete="email" maxLength={254} onChange={(event) => setValue('contactEmail', event.target.value)} type="email" value={values.contactEmail} /></div><div className="field"><label htmlFor={`${mode}-number`}>Contact number <span className="optional-mark">Optional</span></label><input id={`${mode}-number`} autoComplete="tel" type="tel" maxLength={40} onChange={(event) => setValue('contactNumber', event.target.value)} value={values.contactNumber} /></div><div className="field full"><label htmlFor={`${mode}-file`}>Google Drive research file link <span className="optional-mark">Optional</span></label><input id={`${mode}-file`} maxLength={2048} onChange={(event) => setValue('researchFileUrl', event.target.value)} placeholder="https://drive.google.com/..." type="url" value={values.researchFileUrl} /><div className="submission-file-note"><span aria-hidden="true">!</span><strong>Optional. Share a Google Drive or Docs link that the Research Office can open.</strong></div></div></div></section>
      ) : null}

      {step === 4 ? (
        <section className="lifecycle-form-section wizard-panel" ref={panelRef} tabIndex={-1}><div className="wizard-review-heading"><div><h3>Review before submission</h3><p>Check the information below before sending your defense request.</p></div></div><div className="submission-review"><div className="review-card"><div className="review-card-head"><h3>Research</h3><button className="review-edit-button" disabled={submitting} onClick={() => moveTo(1)} type="button">Edit</button></div><dl className="review-grid"><div className="review-full"><dt>Research title</dt><dd>{values.title}</dd></div><div><dt>Program</dt><dd>{values.program}{values.major ? ` - ${values.major}` : ''}</dd></div><div><dt>Research design</dt><dd>{researchDesignLabel(values.researchDesign, values.researchDesignOther)}</dd></div></dl></div><div className="review-card"><div className="review-card-head"><h3>Group</h3><button className="review-edit-button" disabled={submitting} onClick={() => moveTo(2)} type="button">Edit</button></div><ol className="review-member-list">{cleanMembers.map((member, index) => <li key={`${member}-${index}`}>{member}</li>)}</ol><dl className="review-grid review-grid-spaced"><div><dt>Research instructor</dt><dd>{facultyName(faculty, values.instructorId)}</dd></div><div><dt>Research adviser</dt><dd>{facultyName(faculty, values.adviserId)}</dd></div></dl></div><div className="review-card"><div className="review-card-head"><h3>Contact & research file</h3><button className="review-edit-button" disabled={submitting} onClick={() => moveTo(3)} type="button">Edit</button></div><dl className="review-grid"><div><dt>Contact person</dt><dd>{values.contactPerson}</dd></div><div><dt>Email</dt><dd>{values.contactEmail || 'Not provided'}</dd></div><div><dt>Contact number</dt><dd>{values.contactNumber || 'Not provided'}</dd></div><div className="review-full"><dt>Research file</dt><dd>{values.researchFileUrl ? <a className="review-file-link" href={values.researchFileUrl} rel="noopener noreferrer" target="_blank">Open Google Drive file ↗</a> : 'Not provided'}</dd></div></dl></div></div></section>
      ) : null}

      <div className="lifecycle-form-actions">
        <button className="button button-secondary" disabled={submitting} onClick={previousStep} type="button">{step === 1 ? '← Back' : '← Previous'}</button>
...
function ContinueAccess({ faculty, onBack }: { faculty: SubmissionFaculty[]; onBack: () => void }) {
  ...
  async function verify(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      setError(caught instanceof TypeError ? 'Unable to connect. Check your internet connection and try again.' : caught instanceof Error ? caught.message : 'Unable to verify your Research Code. Please try again.')
    } finally {
      setLoading(false)
    }
  }
